// YouTube Playlist Player - Full Dynamic Version
let youtubePlayer;
let currentPlaylist = [];
...
const SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzTAcZnrUdq1T6Q_WevPUzVMRsqupL6i_ST0ue2H8z54lDnHuf8G1QN0eTJGmpUzigA/exec';
...
function createPlayerHTML() {
  ...
  container.innerHTML = `
    <div class="player-container" style="background:white; border-radius:15px; box-shadow:0 20px 60px rgba(183,28,28,0.3); width:100%; height:90vh; margin:0 auto; display:flex; flex-direction:column; overflow:hidden;">
      ...
      <div class="player-body" style="flex:1; display:flex; flex-direction:row; overflow:hidden; min-height:0;">
        <div class="player-video" style="flex:2; background:#000; display:flex; flex-direction:column; min-height:0;">
          ...
          <div class="player-controls" style="background:linear-gradient(to right, #8B0000, #B22222); color:white; padding:10px 15px; display:flex; justify-content:center; gap:15px; border-top:2px solid #DC143C; flex-shrink:0;">
            <button onclick="playPrevVideo()" style="background:#C62828; color:white; border:none; padding:10px 18px; font-size:14px; border-radius:6px; cursor:pointer; font-weight:bold; transition:all 0.3s; display:flex; align-items:center; gap:8px; min-width:120px; justify-content:center;"><span>⏮</span> Προηγούμενο</button>
            <button onclick="playNextVideo()" style="background:#C62828; color:white; border:none; padding:10px 18px; font-size:14px; border-radius:6px; cursor:pointer; font-weight:bold; transition:all 0.3s; display:flex; align-items:center; gap:8px; min-width:120px; justify-content:center;"><span>⏭</span> Επόμενο</button>
          </div>
        </div>
...
function createYouTubePlayer() {
  if (youtubePlayer && youtubePlayer.getIframe) {
    try {
      if (youtubePlayer.getIframe().parentNode) {
        return;
      }
    } catch(e) {}
  }
...
function renderSongsList() {
  ...
  if (!list) {
    console.error('Δεν βρέθηκε το songs-list');
    return;
  }
  ...
  currentPlaylist.forEach((song, index) => {
    ...
    numberSpan.style.cssText = 'color: #666; font-size: 11px; margin-right: 5px; min-width: 20px;';
    numberSpan.textContent = `${index + 1}.`;
    
    const thumbnail = document.createElement('img');
    thumbnail.className = 'player-song-thumb';
    thumbnail.src = song.thumbnail || `https://img.youtube.com/vi/${song.videoId}/default.jpg`;
    ...
    titleDiv.textContent = song.title || 'Χωρίς τίτλο';
...
function loadPlaylist() {
  const url = `${SCRIPT_URL}?playlistId=${currentPlaylistId}`;
  ...
  window[callbackName] = function(data) {
    ...
    if (data && data.songs && data.songs.length > 0) {
      ...
      if (currentPlaylist.length > 0 && youtubePlayer) {
  ...
  script.onerror = function(err) {
    console.error('Σφάλμα φόρτωσης script:', err);
